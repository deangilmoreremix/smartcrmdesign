 ...
 * Enhanced with Supabase Edge Functions integration
 */
...
class AIIntegrationService {
  ...
  async analyzeContact(request: AIAnalysisRequest): Promise<AIAnalysisResponse> {
    ...
    try {
      ...
      const response = await fetch(`${this.supabaseUrl}/functions/v1/ai-analyze`, {
        method: 'POST',
        headers: {
  ...
  async analyzeBulk(request: BulkAnalysisRequest): Promise<BulkAnalysisResponse> {
    ...
    try {
      // Call Supabase Edge Function
      const response = await fetch(`${this.supabaseUrl}/functions/v1/ai-bulk-analyze`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.supabaseAnonKey}`
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(`Bulk analysis failed: ${errorData.error || response.statusText}`);
      }
  ...
  async enrichContact(
    ...
    try {
      ...
      // Call Supabase Edge Function
      const response = await fetch(`${this.supabaseUrl}/functions/v1/ai-enrich`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.supabaseAnonKey}`
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(`Contact enrichment failed: ${errorData.error || response.statusText}`);
      }
  ...
  async getProviderStatus(): Promise<Array<{ name: string; status: 'available' | 'rate_limited' | 'error'; remaining?: number }>> {
    try {
      // Call Supabase Edge Function
      const response = await fetch(`${this.supabaseUrl}/functions/v1/ai-providers-status`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${this.supabaseAnonKey}`
        }
